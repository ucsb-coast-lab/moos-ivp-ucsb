...
void WhoiCommandManager::WriteBinaryData( const string& sCommandParameters )
{
	...
	else
	{
		...
		m_ModemObject.AddRxListener( "CAERR,CACYC", m_vModemReply, m_ReplyTimeStamp,
									 m_ListenSemaphore );
		m_ListenSemaphore.Reset();	// Reset the semaphore
		m_ModemObject.SendNmeaVectorToModem(m_vModemCommand);	// Send command to the modem
		cout << "vModemReply " << endl;
		cout << m_vModemReply.size() << endl;
		cout << "vModemReplied " << endl;
		//cout << m_ListenSemaphore.Wait(3000) << endl;
		// Listen for error or modem to request the data to send
		if ( !m_ListenSemaphore.Wait(3000) )
		{		 
		  std::cout << "Semaphore wait failed...." <<endl;  // njn
		  //  PostToNotifyTarget(sCommandParameters, "Error=Fail");
		  //	return;
		}
		
		// Catch errors signalled by the modem
		//if ( m_vModemReply[0] == "CAERR" )  // njn: original booting version
		if (std::find(m_vModemReply.begin(), m_vModemReply.end(), "CACYC") != m_vModemReply.end()) 
		{
		   std::cout << "Sean found an Error..." << endl;
			PostToNotifyTarget(sCommandParameters, "Error=Fail");
			return;
		}

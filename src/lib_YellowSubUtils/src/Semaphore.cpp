...
bool Semaphore::Wait( int TimeoutMilliseconds )
{
  //std::cout << "using linux version of wait" << std::endl;
  	if (TimeoutMilliseconds == NO_WAIT)
  	{
		return (sem_trywait(&m_Semaphore) == 0);
		 ...
		 std::cout << "waiting forever" << std::endl;
			  std::cout << "returning: " << (sem_wait(&m_Semaphore) == 0);
	...
	else
	{
		...
		//std::cout << "AbsoluteTime = " << AbsoluteTime.tv_nsec << std::endl; 
		//int Count = -1;

	//std::cout << "m_Semaphore = " <<  sem_getvalue(&m_Semaphore, &Count) << std::endl; 
	//	std::cout << "Return value is: " << (sem_timedwait(&m_Semaphore, &AbsoluteTime) == 0) << std::endl;

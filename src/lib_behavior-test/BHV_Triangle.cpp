...
BHV_Triangle::BHV_Triangle(IvPDomain gdomain) : 
  IvPBehavior(gdomain)
{
  this->setParam("descriptor", "bhv_triangle");
...
bool BHV_Triangle::setParam(string param, string val) 
{
  ...
  if((param == "rate") && isNumber(val)) {
    m_desired_rate = vclip_min(dval, 0);
    return(true);
  }
...
IvPFunction *BHV_Triangle::onRunState() 
{
  updateInfoIn();
  ...
  if(ipf)
    ...
    postEMessage("Unable to generate triangle IvP function");
...
bool BHV_Triangle::updateInfoIn()
{
  bool ok1,ok2;
